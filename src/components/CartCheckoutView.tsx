...
export const CartCheckoutView: React.FC<CartCheckoutViewProps> = ({
  ...
  const addressLines = [
    ...
    user?.defaultAddress?.area?.trim(),
    user?.defaultAddress?.city?.trim(),
    user?.defaultAddress?.state?.trim(),
    user?.defaultAddress?.pincode?.trim() ? `PIN: ${user.defaultAddress.pincode.trim()}` : '',
  ].filter(Boolean);

  const recipientName = user?.defaultAddress?.fullName?.trim() || (user?.isLoggedIn ? user?.name : '');
  const recipientPhone = user?.defaultAddress?.phone?.trim() || (user?.isLoggedIn ? user?.phone : '');
  ...
  return (
    <div className={`flex flex-col w-full ${isLoggedIn ? 'pb-28' : 'pb-12'}`}>
      {/* Checkout Header */}
      <div className="px-4 py-2.5 bg-surface-container-lowest shadow-xs border-b border-surface-container flex items-center justify-between">
        ...
        <h1 className="text-[15px] font-bold text-primary">
          {isLoggedIn ? 'Checkout Flow' : 'Your Shopping Cart'}
        </h1>
      ...
      <div className="max-w-2xl mx-auto w-full flex flex-col gap-4">
        ...
        <div className="px-4 flex flex-col gap-4">
          {/* Multi-Step Checkout Stepper - ONLY WHEN SIGNED IN */}
          {isLoggedIn && (
          ...
          {!isLoggedIn && (
            <div className="bg-surface-container-lowest p-5 rounded-xl shadow-xs border border-surface-container flex flex-col items-center text-center gap-3 mt-2">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-[12px] text-on-surface-variant max-w-sm">
                  Please sign in or register your account to view shipping destination, payment methods, and place your order.
                </p>
              </div>
              <button
                onClick={() => onNavigate('account')}
                className="mt-1 px-5 py-2.5 bg-primary text-on-primary rounded-lg text-[13px] font-bold shadow-md hover:bg-primary-container transition-colors"
              >
                Sign In / Register
              </button>
            </div>
          ...
          {/* Student Delivery Destination - ONLY WHEN SIGNED IN */}
          {isLoggedIn && (
            <div className="bg-surface-container-lowest p-4 rounded-xl shadow-xs border border-surface-container flex flex-col gap-2.5">
              ...
              {hasValidAddress ? (
                ...
                <div className="bg-surface-container-low p-3.5 rounded-lg border border-dashed border-surface-container flex flex-col items-center justify-center text-center gap-2">
                  <div className="flex items-center gap-2 text-on-surface-variant text-[13px]">
                    <span className="material-symbols-outlined text-base text-secondary">
                      add_location_alt
                    </span>
                    <span className="font-semibold">No delivery address specified</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="px-3 py-1.5 bg-primary text-on-primary rounded-lg text-[12px] font-bold flex items-center gap-1 hover:bg-primary-container transition-colors shadow-xs"
                  >
          ...
          {/* Payment Method Selection - ONLY WHEN SIGNED IN */}
          {isLoggedIn && (
          ...
          {/* Institutional Guarantee 3-Grid - ONLY WHEN SIGNED IN */}
          {isLoggedIn && (
          ...
          {/* Order Summary Card - ONLY WHEN SIGNED IN */}
          {isLoggedIn && (
      ...
      {/* Sticky Bottom CTA Bar - ONLY WHEN SIGNED IN */}
      {isLoggedIn && (
